...
/* Text $x = Left Cordinate, $y = Top Cordinate, $txt = Print String, $w = Width of text, $t_inc = Next text print increment $y Cordinate */
function MultiText($x, $y, $txt, $w=100, $t_inc=1){
	$cw = &$this->CurrentFont['cw'];
	if($w==0)
		$w = $this->w-$this->rMargin-$this->x;
	$wmax = ($w-2*$this->cMargin);
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			if ($this->unifontSubset) {
				if(!$is_first){$is_first = true;} else {$y += $t_inc;}
				$this->Text($x, $y, mb_substr($s,$j,$i-$j,'UTF-8'));
			}
			else {
				if(!$is_first){$is_first = true;} else {$y += $t_inc;}
				$this->Text($x, $y, substr($s,$j,$i-$j));
			}
			$i++;
			$sep = -1;
			$j = $i;
		...
		if($l>$wmax)
		{
			...
			else
			{
				if ($this->unifontSubset) {
					if(!$is_first){$is_first = true;} else {$y += $t_inc;}
					$this->Text($x, $y, mb_substr($s,$j,$sep-$j,'UTF-8'));
				}
				else {
					if(!$is_first){$is_first = true;} else {$y += $t_inc;}
					$this->Text($x, $y, substr($s,$j,$sep-$j));
				}
				$i = $sep+1;
			}
			$sep = -1;
