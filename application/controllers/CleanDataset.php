...
class CleanDataset extends REST_Controller {
	...
	***************************************************************/
	...
	public function __construct() {
		...
		$this->load->model('CleanDataset_modal');
	}
	...
	public function index_post(){
		...
		if($token_status == TRUE){
			$Pass_Data = array();
			$answer_mc_Ids_Result = $this->CleanDataset_modal->Get_All_answer_mc_MAX_ID();
			if(!empty($answer_mc_Ids_Result)){
				$Ignore_ID_Array = array();
				foreach($answer_mc_Ids_Result as $AMIR){
					$Ignore_ID_Array[] = $AMIR->Max_ID;
				}
				$answer_mc_Delete_Result = $this->CleanDataset_modal->Delete_answer_mc_ID_Not_In($Ignore_ID_Array);
			...
			$answer_desired_Ids_Result = $this->CleanDataset_modal->Get_All_answer_desired_MAX_ID();
			if(!empty($answer_desired_Ids_Result)){
				$Ignore_ID_Array = array();
				foreach($answer_desired_Ids_Result as $ADIR){
					$Ignore_ID_Array[] = $ADIR->Max_ID;
				}
				$answer_desired_Delete_Result = $this->CleanDataset_modal->Delete_answer_desired_ID_Not_In($Ignore_ID_Array);
			...
			$answer_proof_Ids_Result = $this->CleanDataset_modal->Get_All_answer_proof_MAX_ID();
			if(!empty($answer_proof_Ids_Result)){
				$Ignore_ID_Array = array();
				foreach($answer_proof_Ids_Result as $APIR){
					$Ignore_ID_Array[] = $APIR->Max_ID;
				}
				$answer_proof_Delete_Result = $this->CleanDataset_modal->Delete_answer_proof_ID_Not_In($Ignore_ID_Array);
