...
class GetAllCompanies extends REST_Controller {
  /***************************************************************
            ...
            if($value->parent){
             $results1 = $this->GetAllCompanies_model->get_Parent_Company($value->parent);
              if(!empty($results1)){
                ...
                $merge_Array[$key]['parent_id'] = $value->parent;
                $merge_Array[$key]['parent_name'] = $results1[0]->name;
                $merge_Array[$key]['employees'] = $value->total_workforce;
              }
            }else{
                $merge_Array[$key]['id']    = $value->id;
                $merge_Array[$key]['name'] = $value->name;
                $merge_Array[$key]['parent_id'] = null;
                $merge_Array[$key]['parent_name'] = null;
                $merge_Array[$key]['employees'] = $value->total_workforce;
            }
          }
