...
class GetRiskById extends REST_Controller
{
	/***************************************************************
	...
	public function __construct()
	{
		...
		$this->load->model('risk/GetRiskById_model');
	}
	...
	public function index_post()
	{
		...
		if (isset($user_id) && !empty($user_id) && isset($riskId) && !empty($riskId)) {
			...
			if ($token_status == TRUE) {
				$merge_Array = array();
				$results = $this->GetRiskById_model->Get_Risk_By_ID($riskId);
				if (!empty($results)) {
