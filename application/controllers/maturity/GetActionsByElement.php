...
class GetActionsByElement extends REST_Controller {
	...
	***************************************************************/
	...
	public function __construct() {
		...
		$this->load->model('maturity/GetActionsByElement_model');
	}
	...
	public function index_post(){
		...
		if(isset($user_id) && isset($Element_ID)){
			...
			if($token_status == TRUE){
				$merge_Array = array();
				$results = $this->GetActionsByElement_model->get_Action_Results($Element_ID);
				if(!empty($results)){
					$merge_Array["results"] = $results[0]->results;
				}
				$definition = $this->GetActionsByElement_model->get_Action_Definition($Element_ID);
				if(!empty($definition)){
					$merge_Array["definition"] = $definition[0]->definition;
				}
				$measure = $this->GetActionsByElement_model->get_Action_Measure($Element_ID);
				if(!empty($measure)){
					$merge_Array["measure"] = $measure[0]->measure;
				}
				$risk = $this->GetActionsByElement_model->get_Action_Risk($Element_ID);
				if(!empty($risk)){
