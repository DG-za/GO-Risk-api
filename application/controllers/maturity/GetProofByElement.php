...
class GetProofByElement extends REST_Controller {
	/***************************************************************
	...
	***************************************************************/
	...
	public function __construct() {
		...
		$this->load->model('maturity/GetProofByElement_model');
	}
	...
	public function index_post(){
		...
		if(isset($user_id) && isset($Element_ID)){
			...
			if($token_status == TRUE){
				$All_Proof = $this->GetProofByElement_model->Get_Proof_by_Element_ID_Join($Element_ID);
				$Pass_Data = array();
				if(!empty($All_Proof)){
					foreach($All_Proof as $key => $value){
						$merge_array = array("proof" => $value->proof,"type" => $value->type);
