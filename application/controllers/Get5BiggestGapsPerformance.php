...
class Get5BiggestGapsPerformance extends REST_Controller {
	...
	***************************************************************/
	...
	public function __construct() {
		...
		$this->load->model('Get5BiggestGapsPerformance_modal');
	}
	...
	public function index_post(){
		...
		if(isset($user_id)){
			...
			if($token_status == TRUE){
				$Pass_Data = array();
				$merge_array = array();
				$AllElements = $this->Get5BiggestGapsPerformance_modal->GetAllElements_Function();
				foreach ($AllElements as $key => $value) {
					$elementId=$value->id;
					$elementName=$value->name;
					$allPerformanceAnswersByElement = $this->Get5BiggestGapsPerformance_modal->getAllPerformanceAnswersByElement_function($elementId);					
					...
					$allPerformanceDesiredByElement = $this->Get5BiggestGapsPerformance_modal->getAllPerformanceDesiredByElement_function($elementId);
