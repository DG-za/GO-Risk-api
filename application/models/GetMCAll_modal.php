...
class GetMCAll_modal extends CI_Model {
	...
	public function Get_Structured_Answers_By_Element($ID){
		...
		// Old query //  $this->db->select("`answer` as `name`, count(DISTINCT `user`) as `value`, count(`answer`) as `sum`");
		// Old query // $this->db->select("`answer` as `name`,count(`answer`) as `value`,sum(`answer`) as `sum`");
		$this->db->from("`answer_mc`");
		$this->db->like("`element`",$ID);
		// Old query // $this->db->where("`element`",$ID);
